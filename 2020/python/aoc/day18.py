...
def handle_parens(expr, f):
  while '(' in expr:
    ...
    expr = expr[0:start] + str(f(expr[start+1:end])) + expr[end+1:]

  return expr

def eval1(expr):
  expr = handle_parens(expr, eval1)
  ...
  while i < len(ins[1:]):
    ...
    total = total * n if op == '*' else total + (n if op == '+' else -n)
    i += 2

  return total

def eval2(expr):
  expr = handle_parens(expr, eval2)

  return prod([eval(x) for x in expr.split(' * ')])
...
if __name__ == '__main__':
  ...
  res = sum([eval1(line) for line in input])
  print(res)

  res = sum([eval2(line) for line in input])
  print(res)