...
part1 : List (Result Err Model) -> Maybe Int
part1 models =
    models
...
part2 : List (Result Err Model) -> Maybe Int
part2 models =
    models
    ...
    case result of
        Ok model ->
            let
                ...
                matchesLetter =
                    \c -> String.fromChar c == model.letter

                times =
                    List.filter matchesLetter password
                        |> List.length
            in
            times >= model.low && times <= model.high
            ...
            let
                ...
                matchesLetter =
                    \maybeChar ->
                        case maybeChar of
                            Just c ->
                                String.fromChar c == model.letter

                            Nothing ->
                                False

                firstMatches =
                    matchesLetter <| Array.get firstIndex password

                secondMatches =
                    matchesLetter <| Array.get secondIndex password
            in
            (firstMatches && not secondMatches) || (not firstMatches && secondMatches)
...
type alias Inputs =
    { real : List (Result Err Model)
    , example : List (Result Err Model)
    }


getInputs : Inputs
getInputs =
    { real = getRealInput |> List.map toModel
    , example = getExampleInput |> List.map toModel
    }
...
toModel : String -> Result Err Model
toModel =
    run modelParser
