...
type Field
    ...
    | IssueYear Int
    | ExpirationYear Int
    | Height Int (Maybe Measurement)
    | HairColor (Maybe Hex)
    | EyeColor (Maybe EyeColorValue)
...
toEyeColor : Maybe String -> Field
toEyeColor maybeColor =
    EyeColor (toEyeColorValue maybeColor)


toEyeColorValue : Maybe String -> Maybe EyeColorValue
toEyeColorValue maybeColor =
    case maybeColor of
        Just color ->
            if color == "amb" then
                Just Amber

            else if color == "blu" then
                Just Blue

            else if color == "brn" then
                Just Brown

            else if color == "gry" then
                Just Grey

            else if color == "grn" then
                Just Green

            else if color == "hzl" then
                Just Hazel
...
{-| Count the passports that have all the required fields.
-}
part1 : List Passport -> Int
part1 passports =
    passports
        |> List.filter hasAllRequiredFields
        |> List.length


hasAllRequiredFields : Passport -> Bool
hasAllRequiredFields passport =
    if List.length passport == 8 then
        True

    else if List.length passport == 7 && List.all isNotCompanyField passport then
        True
...
isNotCompanyField : Field -> Bool
isNotCompanyField field =
    case field of
        CompanyId _ ->
            False
...
{-| Count the passports that have all the required fields
AND each field is valid.
-}
part2 : List Passport -> Int
part2 passports =
    passports
        |> List.filter isValid
        |> List.length
...
isValid passport =
    hasAllRequiredFields passport && allFieldsAreValid passport


allFieldsAreValid : Passport -> Bool
allFieldsAreValid passport =
    passport
...
isFieldValid field =
    case field of
        ...
        Height height maybeMeasurement ->
...
type alias FieldResult =
    Result (List DeadEnd) (List Field)


toPassports : List String -> List Passport
toPassports input =
    List.map (run passportParser) input
        |> withoutErrors


withoutErrors : List FieldResult -> List Passport
withoutErrors results =
    withoutErrorsRecur [] results


withoutErrorsRecur : List Passport -> List FieldResult -> List Passport
...
passportParser =
    loop [] passportParserHelp


passportParserHelp : List Field -> Parser (Step (List Field) (List Field))
passportParserHelp revFields =
    oneOf
        [ succeed (\fld -> Loop (fld :: revFields))
            |= fieldParser
            |. spaces
        , succeed ()
...
passportIdParser =
    succeed PassportId
        |. symbol "pid:"
        |= maybe (oneOrMore "pid digit" Char.isDigit)
        |. zeroOrMore notSpace
...
unknownParser : Parser Field
unknownParser =
    succeed Unknown
        |= oneOrMore "unknown alpha" Char.isAlpha
        |. symbol ":"
        |= oneOrMore "unknown" notSpace
...
type alias Inputs =
    { real : List Passport
    , example : List Passport
    }


getInputs : Inputs
getInputs =
    { real = getRealInput |> toFields |> toPassports
    , example = getExampleInput |> toFields |> toPassports
    }
...
toFields : List String -> List String
toFields input =
    toFieldsRecur [] "" input
        |> List.map String.trim


toFieldsRecur : List String -> String -> List String -> List String
toFieldsRecur finalFields fields remaining =
    if List.isEmpty remaining then
        finalFields
        ...
        let
            first =
                Maybe.withDefault "" <| List.head remaining
        ...
        if String.isEmpty first then
            let
                nextFinalFields =
                    finalFields ++ [ fields ]
            in
            toFieldsRecur nextFinalFields "" rest
            ...
            let
                nextPassportAcc =
                    fields ++ " " ++ first
            in
            toFieldsRecur finalFields nextPassportAcc rest
...
prettyPassport : Passport -> String
prettyPassport passport =
    String.fromInt (List.length passport)
        ++ " "
        ++ (passport
...
prettyField field =
    case field of
        BirthYear yr ->
            "byr: " ++ String.fromInt yr

        IssueYear yr ->
            "iyr: " ++ String.fromInt yr

        ExpirationYear yr ->
            "eyr: " ++ String.fromInt yr
                ...
                mstr =
                    case m of
                        ...
                        Nothing ->
                            ""
            in
            "hgt: " ++ String.fromInt h ++ mstr

        HairColor color ->
            "hcl: " ++ Maybe.withDefault "invalid" color
        ...
        PassportId maybeId ->
            "pid: " ++ Maybe.withDefault "invalid" maybeId
